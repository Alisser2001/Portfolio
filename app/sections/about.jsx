...
export default function About() {
    ...
    return (
        <section ref={refAbout} className={`flex flex-col w-full px-20 md:px-0 md:w-1/2 min-h-screen h-auto justify-center items-start text-white opacity-0 ${isVisibleAbout ? 'animate-fade-bottom-y-4' : ''}`} id='about' style={{animationDelay: ".2"}}>
            <div className={`flex flex-row w-full h-auto justify-end items-center md:px-10 mb-32  ${isVisibleAbout ? 'animate-fade-bottom-y-1' : ''}`}>
                <div className='h-1 w-1/2 md:w-2/3 bg-[#2E28A0] mr-5'></div>
                <h1 className='font-bold text-xl md:text-2xl lg:text-3xl xl:text-4xl w-1/2 md:w-1/3 flex justify-center items-center'><span className='text-[#2E28A0] mr-5'>01.</span> About Me</h1>
            </div>
            <div className='flex flex-col-reverse lg:flex-row w-full h-auto justify-start lg:justify-between items-center'>
